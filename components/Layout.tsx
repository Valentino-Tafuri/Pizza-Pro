...
import { 
  ...
  Warehouse, Tag, ScanBarcode, TrendingUp, Star, MapPin, FileText, Upload
} from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeView, setActiveView, title }) => {
  ...
  const [isLaboratorioExpanded, setIsLaboratorioExpanded] = useState(false);

  // Auto-expand settings menu when a sub-section is active
  useEffect(() => {
    if (activeView === 'settings' || activeView === 'settings-assets' || activeView === 'settings-staff' || activeView === 'settings-suppliers' || activeView === 'settings-extensions' || activeView === 'profile') {
  ...
  // Auto-expand laboratorio menu when a sub-section is active
  useEffect(() => {
    if (activeView === 'laboratorio' || activeView === 'laboratorio-prefermenti') {
      setIsLaboratorioExpanded(true);
    } else {
      setIsLaboratorioExpanded(false);
    }
  }, [activeView]);

  // Auto-expand inventario menu when a sub-section is active
  useEffect(() => {
    if (activeView === 'inventario' || activeView === 'prep-settings' || activeView === 'warehouse' || activeView === 'fifo-labels' || activeView === 'custom-labels' || activeView === 'scan' ||
  ...
  useEffect(() => {
    if (activeView === 'marketing' || activeView === 'marketing-overview' || activeView === 'marketing-google') {
  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { 
          id: 'laboratorio' as ViewType, 
          label: 'Laboratorio', 
          icon: Calculator,
          hasSubmenu: true,
          subItems: [
            { id: 'laboratorio-prefermenti' as ViewType, label: 'Prefermenti', icon: Beaker },
          ]
        },
      ]
    },
      ...
      items: [
        { 
          ...
          subItems: [
            ...
            { id: 'settings-extensions' as ViewType, label: 'Integrazioni', icon: Settings },
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100">
      ...
      <div className="flex-shrink-0 p-6 pb-3">
        <div className="flex items-center space-x-2 mb-6">
          ...
          <span className="text-lg font-black tracking-tighter">Pizza Pro</span>
        </div>
      </div>

      {/* Navigation - Scrollable */}
      <nav className="flex-1 px-6 pb-4 min-h-0 overflow-y-auto">
        <div className="space-y-5">
          {navGroups.map((group) => (
            <div key={group.label} className="space-y-1.5">
              <h3 className="px-3 text-[9px] font-black uppercase tracking-widest text-gray-300">
                {group.label}
              </h3>
              <div className="space-y-0.5">
                {group.items.map((item) => {
                  const Icon = item.icon;
                  const isSettingsSubActive = activeView === 'settings-assets' || activeView === 'settings-staff' || activeView === 'settings-suppliers' || activeView === 'settings-extensions' || activeView === 'profile';
                  ...
                  const isLaboratorioSubActive = activeView === 'laboratorio-prefermenti';
                  const isActive = activeView === item.id || (item.hasSubmenu && item.id === 'settings' && isSettingsSubActive) || (item.hasSubmenu && item.id === 'inventario' && isInventarioSubActive) || (item.hasSubmenu && item.id === 'marketing' && isMarketingSubActive) || (item.hasSubmenu && item.id === 'laboratorio' && isLaboratorioSubActive);
                  const isExpanded = item.hasSubmenu && (
                    ...
                    (item.id === 'laboratorio' && (isLaboratorioExpanded || isLaboratorioSubActive))
                  );
                  ...
                  return (
                    <div key={item.id}>
                      <button
                          onClick={() => {
                            if (item.hasSubmenu) {
                              ...
                              } else if (item.id === 'laboratorio') {
                                // Laboratorio non apre nulla, solo espande/contrae
                                setIsLaboratorioExpanded(!isLaboratorioExpanded);
                              }
                            } else {
                      ...
                      {item.hasSubmenu && isExpanded && item.subItems && (
                        <div className="ml-3 mt-0.5 space-y-0.5 border-l-2 border-gray-100 pl-1.5">
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center space-x-2.5">
                                  ...
                                  <span className="text-[11px] font-black tracking-tight">{subItem.label}</span>
                                </div>
                                {isSubActive && <ChevronRight size={11} className="opacity-40" />}
                              </button>
                            );
                          })}
                        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] flex">
      ...
      <aside className="hidden lg:block w-72 h-screen fixed top-0 left-0 z-30">
        <SidebarContent />
      </aside>
